...
- フィード定期取得を `apalis` に移行（リトライ・バックオフ・per-feed スケジュール）。`shared/scheduler.rs` を差し替える。**← 着手中（方式B: cron + Postgres ジョブキュー）**
- 記事本文の抽出強化（現状はフィードの content/summary をそのまま使用）。
- 全文検索（PostgreSQL の `tsvector`）。
- 未読/既読フィルタ UI、フィード単位の絞り込み。

### ユーザー要望の機能バックログ（2026-06-26 起票）

別セッションでユーザーが挙げた機能要望。優先度・着手順は未確定。実装時は Vertical Slice 方針（新スライス1枚 + `features/mod.rs` に `.merge()`）に沿うこと。

- **フィード管理機能** — フィードの一覧・編集・削除を行う管理画面。
- **フィードのフォルダ分け** — フィードをフォルダ（カテゴリ）で整理。
- **フィード管理に投稿状況を表示** — 最終投稿からの経過日数と投稿頻度を表示。
- **ダークテーマ切り替え** — ライト/ダークを切り替え可能に（`app.css` の oklch トークンを活用）。
- **Instapaper 連携** — Instapaper への連携機能。
- **後で読む機能** — Instapaper をバックエンドにした「後で読む」。
- **デザインのミニマル化** — 視認性を上げる UI 改善。
- **フィード追加 UI の配置見直し** — 画面最上部固定をやめ、使用頻度・シーンに応じて配置変更。
- **既読管理** — 記事の既読/未読を管理（バックエンドの既読 API は実装済み、UI 等が主）。
- **2 ペインのリーダーレイアウト** — 左: フォルダ/フィード、右: 記事本文。
- **左ペインの「すべて / 未読のみ」切り替え** — フィード一覧の表示フィルタ。

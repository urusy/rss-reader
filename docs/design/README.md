...
| 29 | Google Reader/Fever 同期API | 将来(stub) | L〜XL | 要 | **14** | [29](./29-sync-api.md) |
...
> **29–33 はスタブ**（約1ページ）。着手前に各設計を本格詳細化すること。
...
> **第2弾内ハード依存**: `25→24`・`29→14` のみ。`24 タグ` は 23/25/27/28 の事実上の前提（早めに）。`13 全文抽出` は全 AI の品質上限を上げる土台（最優先）。

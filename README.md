...
- **自己ホスト前提**: 認証は任意（LAN 内前提）、鍵・秘密は環境変数で外挿。
...
| `AUTH_TOKEN` | （空） | 設定すると `/api` 全体が `Authorization: Bearer <token>` 必須（health と `/api/auth/*` は除外）。未設定なら認証無効 |
...
- `AUTH_TOKEN` を設定して `/api` を保護する（`openssl rand -base64 32`）。
- `CorsLayer::permissive()`（`features/mod.rs`）を実際のフロントオリジンに絞る。
- リバースプロキシで TLS 終端・セキュリティヘッダ（CSP 等）を付与する。
...
すべて `/api` 以下。`AUTH_TOKEN` 設定時は health と `/api/auth/*` を除き `Authorization: Bearer` が必要です。代表的なエンドポイント:

| 分類 | 代表エンドポイント |
|------|--------------------|
| ヘルス | `GET /api/health` `GET /api/health/db` |
| 認証 | `POST /api/auth/login` `GET /api/auth/status` |
